from pydantic import BaseModel, Field, ValidationError, model_validator
from typing import List
from enum import Enum
from datetime import datetime


#ENUM

class Rank(str, Enum):
    cadet = "cadet"
    officer = "officer"
    lieutenant = "lieutenant"
    captain = "captain"
    commander = "commander"


# CREW 
class CrewMember(BaseModel):
    member_id: str = Field(..., min_length=3, max_length=10)
    name: str = Field(..., min_length=2, max_length=50)
    rank: Rank
    age: int = Field(..., ge=18, le=80)
    specialization: str = Field(..., min_length=3, max_length=30)
    years_experience: int = Field(..., ge=0, le=50)
    is_active: bool = True


#SPACE MISSION  MY LOVE
class SpaceMission(BaseModel):
    mission_id: str = Field(..., min_length=5, max_length=15)
    mission_name: str = Field(..., min_length=3, max_length=100)
    destination: str = Field(..., min_length=3, max_length=50)
    launch_date: datetime
    duration_days: int = Field(..., ge=1, le=3650)
    crew: List[CrewMember] = Field(..., min_length=1, max_length=12)
    mission_status: str = "planned"
    budget_millions: float = Field(..., ge=1.0, le=10000.0)

    @model_validator(mode="after")
    def validate_mission(self):
        # ID start with M
        if not self.mission_id.startswith("M"):
            raise ValueError("Mission ID must start with 'M'")

        #  at least 1 commander, pelo menos 1 comandante ou capitão
        has_lead = any(
            member.rank in (Rank.captain, Rank.commander)
            for member in self.crew
        )
        if not has_lead:
            raise ValueError("Mission must have at least one Commander or Captain")

        # long mission need 50% experiences, longa precisa de 50% experientes (>=5 anos)
        if self.duration_days > 365:
            experienced = sum(1 for m in self.crew if m.years_experience >= 5)
            if experienced < len(self.crew) / 2:
                raise ValueError("Long missions require 50% experienced crew (5+ years)")

        #  todos os membros têm de estar ativos, all members must be a live
        if not all(member.is_active for member in self.crew):
            raise ValueError("All crew members must be active")

        return self



def main():
    print("Space Mission Crew Validation")
    print("=" * 40)

    # valid mission
    try:
        mission = SpaceMission(
            mission_id="M2024_MARS",
            mission_name="Mars Colony Establishment",
            destination="Mars",
            launch_date="2026-05-01T10:00:00",
            duration_days=900,
            crew=[
                CrewMember(
                    member_id="C001",
                    name="Sarah Connor",
                    rank="commander",
                    age=40,
                    specialization="Leadership",
                    years_experience=15,
                ),
                CrewMember(
                    member_id="C002",
                    name="John Smith",
                    rank="lieutenant",
                    age=35,
                    specialization="Navigation",
                    years_experience=6,
                ),
                CrewMember(
                    member_id="C003",
                    name="Alice Johnson",
                    rank="officer",
                    age=30,
                    specialization="Engineering",
                    years_experience=5,
                ),
            ],
            budget_millions=2500.0,
        )

        print("Valid mission created:")
        print(f"Mission: {mission.mission_name}")
        print(f"ID: {mission.mission_id}")
        print(f"Destination: {mission.destination}")
        print(f"Duration: {mission.duration_days} days")
        print(f"Budget: ${mission.budget_millions}M")
        print(f"Crew size: {len(mission.crew)}")
        print("Crew members:")

        for c in mission.crew:
            print(f"- {c.name} ({c.rank})")

    except ValidationError as e:
        print(e)

    print("=" * 40)

    # invalid mission (without commander/captain)
    try:
        bad_mission = SpaceMission(
            mission_id="M_FAIL_01",
            mission_name="Broken Mission",
            destination="Moon",
            launch_date="2026-05-01T10:00:00",
            duration_days=100,
            crew=[
                CrewMember(
                    member_id="C010",
                    name="Bob",
                    rank="officer",
                    age=30,
                    specialization="Science",
                    years_experience=2,
                )
            ],
            budget_millions=100.0,
        )

    except ValidationError as e:
        print("Expected validation error:")
        print(e)


if __name__ == "__main__":
    main()